...
def obtener_menu_texto() -> str:
    response = supabase.table("menu")\
        .select("nombre, categoria, calorias, proteinas, carbohidratos, grasas, alergenos, apto_vegetariano, apto_vegano, apto_sin_gluten, apto_diabetes, apto_hipertension, precio")\
        .eq("disponible", True)\
        .execute()
